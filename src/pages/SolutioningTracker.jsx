...
import { getContentDevTracker as getSolutioningTracker, addContentDevRow as addSolutioningRow, updateContentDevRow as updateSolutioningRow, deleteContentDevRow as deleteSolutioningRow } from '../services/api.js';
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function SolutioningTracker({ onRefreshed }) {
  ...
  const load = async () => {
    ...
    try {
      const rows = await getSolutioningTracker();
      setData(rows);
  ...
  const handleEdit = async form => {
    ...
    setSaving(true);
    try {
      await updateSolutioningRow(editRow.sno, form);
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      await deleteSolutioningRow(deleteRow.sno);
  ...
  if (loading) return <LoadingState message="Loading Solutioning Tracker from OneDrive…" />;
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid var(--border)' }}>
                {['#', 'Client', 'Program Type', 'Start Date', 'End Date', 'POC', 'EV Req.', 'PM Req.', 'Due Date', 'Completion Date', 'Status', canEdit('solutioning') ? 'Actions' : null]
                  .filter(Boolean)
                  .map(h => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={canEdit('solutioning') ? 12 : 11} style={{ padding: 32, textAlign: 'center', color: 'var(--muted)' }}>
                    No entries found
                  </td>
                </tr>
              ) : filtered.map((r, i) => (
